...
We ran our modified mean shift algorithm on this initial dataset to create the correction vector fields shown in Figure \ref{fig:image-table}. Our MATLAB implementation took under 5 minutes for each celebrity and has the dimensions of 460x320. The great majority of the time was spent in un-optimized nearest neighbor search. In Figure \ref{daf-convergence} we plot how our correction vector field converges for Borack Obama, by calculating the vector field for a subset of the drawings. There appears to be an inflection point around 25 drawing, at which point the correction vector field would probably work well. However every drawing still appears to improve the correction field, and even with 131 drawings the correction vector field has not completely converged. 
...
Likewise we observe a significant change in the average distance to the consensus drawings. In figure  It increases by 18\% when our auto-corrector has turned on with a (p=1.65e-06), suggesting that artists do not need to draw as precisely. Figure \ref{fig:daf-distance} show the average distance to the consensus for each celebrity.

\begin{figure}
  \centering%
\includegraphics[width=3.5in]{./figures/userstudy/distanceToConsensus.pdf}
  \caption{The average distance to the consensus for each celebrity.  fix caption}
